...
## 📱 iOS Development & Testing (Mac SSH)

This project uses a MacinCloud Mac for iOS development and testing when developing on Windows.

### Mac SSH Connection Details

Credentials are stored in `.env`:
```bash
MAC_SSH_HOSTNAME="NY465.macincloud.com"
MAC_SSH_IP="74.91.11.165"
MAC_SSH_USERNAME="user289978"
MAC_SSH_PASSWORD="cnd77867fdh"
```

### Connecting to Mac

```bash
# SSH into Mac for iOS development
ssh user289978@NY465.macincloud.com
# Or use IP if hostname doesn't resolve:
ssh user289978@74.91.11.165
```

### iOS Build & Test Workflow

1. **Clone/Sync Project on Mac:**
   ```bash
   # On Mac, clone the repo or sync changes
   git clone https://github.com/yourrepo/backtrack.git
   cd backtrack
   npm install
   ```
...
3. **Build for Physical Device (requires Apple Developer account):**
   ```bash
...
1. **Using Expo Go (quickest):**
   - Start server: `npx expo start`
   - Scan QR code with Expo Go app on iPhone

2. **Development Build (recommended for full features):**
   - Build: `npx eas build --platform ios --profile development`
   - Install the generated .ipa on device

3. **iOS Simulator:**
   - Run: `npx expo start --ios`
   - Uses Xcode iOS Simulator

### Background Location Testing on iOS

For testing background location features (always-on tracking):
1. Enable location services in iOS Settings
2. Grant "Always Allow" location permission to the app
3. Enable background app refresh for the app
4. Test by moving between locations with the app backgrounded
...
# Open Android Studio -> Tools -> Device Manager -> Start
```
...
### Testing Background Location on Android

1. Grant location permissions (including "Allow all the time")
2. Disable battery optimization for the app
3. Test with app in background and screen off
4. Check notification shows "Location tracking is active"
