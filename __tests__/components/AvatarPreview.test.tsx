...
vi.mock('@/lib/avatar/dicebear', () => ({
  createAvatarDataUri: vi.fn((config: AvatarConfig, size: number) => {
...
describe('AvatarPreview', () => {
  ...
  describe('re-renders when config changes', () => {
    it('re-renders with new data URI when config changes', () => {
      ...
      const initialCallCount = (createAvatarDataUri as Mock).mock.calls.length

      rerender(<AvatarPreview config={{ topType: 'Hat' }} />)

      expect((createAvatarDataUri as Mock).mock.calls.length).toBeGreaterThan(initialCallCount)
  ...
  describe('memoization behavior', () => {
    it('does not re-render when same config object reference is passed', () => {
      ...
      expect((createAvatarDataUri as Mock).mock.calls.length).toBe(initialCallCount)
    })
