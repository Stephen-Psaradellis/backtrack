/**
 * Meetup OAuth Callback Route
 *
 * Handles the OAuth callback from Meetup after user authorization.
 * Exchanges the authorization code for access tokens and stores them
 * securely in the user_event_tokens table.
 *
 * Flow:
 * 1. User clicks "Connect Meetup" and is redirected to Meetup
 * 2. User authorizes the app on Meetup
 * 3. Meetup redirects back to this route with an authorization code
...
const SUCCESS_REDIRECT = '/events?connected=meetup'
const ERROR_REDIRECT = '/events?error=meetup_auth_failed'
const LOGIN_REDIRECT = '/login?redirect=/events'

/**
 * GET /api/auth/meetup
 *
 * Handles the OAuth callback from Meetup.
 *
 * Query Parameters:
 * - code: Authorization code from Meetup
...
export async function GET(request: NextRequest) {
  ...
  // Handle error response from Meetup
  if (error) {
  ...
  try {
    ...
    // Handle specific Meetup API errors
